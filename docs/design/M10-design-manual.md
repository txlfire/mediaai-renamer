...
当前实现中，`source_priority`、`field_completeness`、`merge_key`、`field_sources` 写入候选 `raw_data`，用于后续前端展示和真实 Provider 接入。
...
截至 `0.10.2`，多源候选仍缓存在 `rename_previews.metadata_candidates_json` 中，未新增独立候选缓存字段。
...
截至 `0.10.2`，前端多源入口尚未接入，以上为后续实现要求。
