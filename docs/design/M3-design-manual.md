...
- 多选执行：勾选命名预览记录后点击“执行重命名”。
- 全部执行：点击“全部重命名”，处理当前查询结果中状态允许重命名的记录。
- 单条执行：在操作列点击“执行重命名”图标，仅处理当前行。
- 空目标文件名拦截：执行前先检查目标文件名是否为空，空名条目不会进入 dry-run。
- 一键剔除：空名弹窗支持“剔除空名条目并继续”，剔除后对剩余条目继续执行 dry-run。
- 后端兜底：dry-run 对空目标文件名标记为 `conflict`，不抛出路径异常。
