...
当前阶段：M10 开发中，已完成 M10-2 Bangumi 后端补充源；M9 已发布 `v0.9.0`，M9+ 维护版 `0.9.1` 已补齐任务治理归档入口。
...
- 多源元数据后端：支持 TMDB 与 Bangumi 补充源编排，Bangumi 可选 Access Token、真实连接测试、动画条目搜索、失败隔离和统一候选映射；前端入口仍在后续阶段开发。
