...
当前阶段：M10 已正式发布 `v0.10.7`；M11 已完成远程协议扩展开发规划，并推进到 M11-1C WebDAV MOVE dry-run 基础接入。M9 已发布 `v0.9.0`，M9+ 维护版 `0.9.1` 已补齐任务治理归档入口。
...
- WebDAV 已支持 HTTPS 地址校验、连接测试、目录浏览、递归扫描和 MOVE dry-run 冲突校验；真实重命名暂未开放。FTP、SFTP、S3 暂不支持真实扫描和重命名，仅作为后续候选协议。
