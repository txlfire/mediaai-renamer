...
- 安全重命名 M3 初版：支持多选重命名、全部重命名、单条重命名、dry-run 冲突检测、空目标文件名拦截、一键剔除空名条目、确认重命名入口和执行结果记录。
- 统一热配置：支持通过“系统设置”热更新 TMDB 配置和最小扫描文件阈值，无需重启服务。
- TMDB 元数据匹配：支持 TMDB V4 只读访问令牌优先调用，V4 失败时自动降级到 V3 API Key；支持单条命名预览匹配、匹配度展示、高置信自动回填、低置信候选弹窗和人工选择回填。
- 文件运维：低于最小扫描阈值的视频文件进入待处理列表，支持单条移除、清空列表和批量迁移。
...
该命令会清理当前进程内重复的 `Path/PATH` 环境变量，避免 Windows 后台启动时报“字典关键字重复”；同时会把前后端日志写入 `.codex/run-logs/`。
...
- `minimum_file_size`：最小扫描文件阈值，当前通过系统设置页热更新，默认 `0`。
...
- M4 开发计划：[docs/superpowers/plans/2026-06-26-m4-tmdb-hot-config-implementation.md](docs/superpowers/plans/2026-06-26-m4-tmdb-hot-config-implementation.md)
- M4 设计手册：[docs/design/M4-design-manual.md](docs/design/M4-design-manual.md)
- M4 用户手册：[docs/manuals/M4-user-manual-cn.md](docs/manuals/M4-user-manual-cn.md)
