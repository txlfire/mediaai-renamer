...
当前阶段：M10 开发中，已完成 M10-4 豆瓣代理后端补充源；M9 已发布 `v0.9.0`，M9+ 维护版 `0.9.1` 已补齐任务治理归档入口。
...
- 多源元数据后端：支持 TMDB、Bangumi、TVDB 与用户自备豆瓣代理补充源编排，Bangumi 可选 Access Token、真实连接测试、动画条目搜索；TVDB 支持官方登录、剧集搜索、季集和 episode title 补充；豆瓣代理按用户配置 Base URL 查询并映射统一候选；前端入口仍在后续阶段开发。
