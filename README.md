...
当前阶段：M10 已正式发布 `v0.10.7`；M11 已推进到 M11-1F，完成 WebDAV 扫描、重命名、回滚和失败恢复闭环，下一步进行协议集成环境验证。M9 已发布 `v0.9.0`，M9+ 维护版 `0.9.1` 已补齐任务治理归档入口。
...
- 多源元数据：支持 TMDB、Bangumi、TVDB 与用户自备豆瓣代理补充源编排，Bangumi 可选 Access Token、真实连接测试、动画条目搜索；TVDB 支持官方登录、剧集搜索、季集和 episode title 补充；豆瓣代理按用户配置 Base URL 查询并映射统一候选；系统设置可维护补充源配置，重命名页可执行单条、选中和当前列表多源匹配并查看字段来源。
...
- WebDAV 已支持 HTTPS 地址校验、连接测试、目录浏览、递归扫描、MOVE dry-run、真实重命名、反向 MOVE 回滚和失败恢复。FTP、SFTP、S3 暂不支持真实扫描和重命名，继续按 M11 计划推进。
